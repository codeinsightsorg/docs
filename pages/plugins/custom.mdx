...
A plugin is a class that implements the `BasePlugin` interface. This interface defines the following properties:
| Property                 | Type                                            | Description                                      |
|--------------------------|-------------------------------------------------|--------------------------------------------------|
| onFinishProcessing       | `() => PluginAnalyzedEntity[]`                  | Callback function called when processing of all files finishes. |
| onAllFinishProcessing    | `(items: AnalyzeResults, plugin: BasePlugin) => any` | Callback function called when processing of **all plugins** finishes.  |
...
| parser                   | `Parser`                                        | One of the supported parsers (TypeScript, HTML, or JSON)|
...
Start by creating a class with your plugin name. The plugin needs to implement one of the following interfaces:
